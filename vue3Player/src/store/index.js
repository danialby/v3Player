...
export const playerStore = defineStore('player', {
    ...
    actions: {
        ...
        setPlayerAdvData(state, playerAdvData) {
            this.playerAdvData = playerAdvData
        },
        setPlayerAdvertiseData(state, playerAdvertiseData) {
            this.playerAdvertiseData = playerAdvertiseData
        },
        // setFullAnimationContainer(state, FAC_state) {
        //     this.fullAnimationContainer = FAC_state
        // },
        setPlayerLyrics(playerLyrics) {
            // let lyrics
            if(playerLyrics) {
                ...
                this.playerLyrics = JSON.parse(data)
            }
            else {
    ...
    if (!endOfPlaylist.value) {
        ...
        initPlayingObject(PlayerIndex,playingData)    {
            ...
            this.endOfPlaylist = PlayerIndex === this.playerTracks.tracks.length - 1
            ...
            console.log(theMp3)
            this.setFetchUrl(this.cdnUrl + theMp3[0].name +
                '?type=pwa&melodify_token=' + userObject.user.id + '&download_token=' + userObject.token)


            if (document.getElementById('Player').scrollTop > 10 && (!this.playerLyrics && !this.isLoadingLyrics)) {
                if (this.playerData.lyric.has_lyric) {
                    this.toggleLyrics()
                }
            }
...
export const melodify = defineStore('melodify', {
    ...
    actions: {
        ...
        //   this.lastEditedCollection = collection
        // },
        ...
        //     this.initialData.telegram_data.has_connected = true
        //     // window.open(telegramLink)
        // },
        // setTGStatus(state, tgStatus) {
        //         this.initialData.telegram_data.has_connected = tgStatus.telegram_data.has_connected
        //         this.initialData.telegram_data.connected_title = tgStatus.telegram_data.connected_title
        //         this.isTGUnlinked = !tgStatus.telegram_data.has_connected;
        // },
        // setTGUnlinked(state,unlinked)
        // {
        //     this.isTGUnlinked = unlinked
        // },
        // unsetTGStatus(state,unset) {
        //     if(!unset) {
        //         this.initialData.telegram_data.has_connected = false
        //         this.initialData.telegram_data.connected_title = null
        //     }
        // },
        // musicSentToTelegram(state,sendResult)
        // {
        //         this.telegramLimitationObj = sendResult
        // },
        ...
        setUpdateDialog(state,appUpdateData) {
            this.appUpdateDialogData = appUpdateData
        },
        ...
        setInitialData(state, initialData) {
            ...
            let device_token = this.devUserObject["09353264254"]
            ...
            if (device_token == null) {
                for (let theItem in theInitialData) {
                    ...
                    this[theItem] = initialData[theItem]
                }
            } else {
                for (let theItem in theInitialData) {
            ...
            this.device_token = initialData.device_token
            ...
            this.limitations = JSON.parse(localStorage.getItem("limitations"))


            this.lyric_limitation_data =  JSON.parse(localStorage.getItem("lyric_limitations_data"))

            let lyric_report_types = JSON.parse(localStorage.getItem("lyric_report_types"))
            this.lyric_reports_for_sync = lyric_report_types.filter(item => item.only_for_sync)
            this.lyric_reports_for_nonSync = lyric_report_types.filter(item => !item.only_for_sync)

            let stream_limitation = JSON.parse(localStorage.getItem("limitations"))
            this.stream_limitation_data  = stream_limitation.filter(item => item.limitation_key.includes('stream_quality_')).reverse()

            let download_limitation = JSON.parse(localStorage.getItem("limitations"))
            this.download_limitation_data  = download_limitation.filter(item => item.limitation_key.includes('download_quality_')).reverse()
        ...
        toggleLikedTracks(state, track_id) {
            let isInLikedTracks = this.userActivity.liked_tracks.filter(item => item === track_id)
            if(isInLikedTracks.length === 0) {
                this.userActivity.liked_tracks.push(track_id)
            }
            else {
                let index =  this.userActivity.liked_tracks.indexOf(track_id)
                if (index !== -1) {
                    this.userActivity.liked_tracks.splice(index, 1);
                }
            }
        },
        toggleFollowedArtists(state, artist_id) {
            let isInFollowedArtists = this.userActivity.following_artists.filter(item => item === artist_id)
            if(isInFollowedArtists.length === 0) {
                this.userActivity.following_artists.push(artist_id)
            }
            else {
                let index =  this.userActivity.following_artists.indexOf(artist_id)
                if (index !== -1) {
                    this.userActivity.following_artists.splice(index, 1);
                }
            }
        },
        toggleFollowedCollections(state, collection_id) {
            let isInFollowedCollections = this.userActivity.following_collections.filter(item => item === collection_id)
            if(isInFollowedCollections.length === 0) {
                this.userActivity.following_collections.push(collection_id)
            }
            else {
                let index =  this.userActivity.following_collections.indexOf(collection_id)
                if (index !== -1) {
                    this.userActivity.following_collections.splice(index, 1);
                }
            }
        },
        setDialogs(state, dialogs) {
            this.appDialogs = dialogs
        },
            ...
            if (payload._items) {
                ...
                if (action === 'loadmore') {
                    if (!_items.end && _items.remaining !== 0) {
                        if (this[payload.storeObject][payload.storeType].length !== 0) {
                            this[payload.storeObject].end = _items.end
                            this[payload.storeObject].remaining = _items.remaining
                            this[payload.storeObject][payload.storeType].push(..._items[payload.storeType])
                        } else {
                            this[payload.storeObject] = _items
                            this[payload.storeObject][payload.storeType].push(..._items[payload.storeType])
                        }
                    ...
                    this.recyclingObject.isRecycling = false
                } else if (action === 'default') {
                    this[payload.storeObject] = _items
                    this[payload.storeObject][payload.storeType] = _items[payload.storeType]
                }
            } else {
                this[payload.storeObject] = payload._items
            }
        },
        ...
        setTabState(state, tabState) {
            this.tabState.isPresent = tabState
        },
        hasDHint(state, tf) {
            this.hasDHint = tf
        },
        navButtonHandler(state, hasRightLeft) {
            this.hasRightLeft.right = hasRightLeft[0]
            this.hasRightLeft.left = hasRightLeft[1]
        },
        setNavState(state, navStyle) {
            this.navState.backColor = navStyle[0]
            this.navState.title = navStyle[1]
            this.navState.shadow = navStyle[2]
            this.navState.rdp = navStyle[3]
            this.navState.ldp = navStyle[4]
        },
        // setPlaylistEditMode(state, plEditMode) {
        //     this.isEditingMode = plEditMode
        // },
        setMiddleState(state, middleState) {
            this.middleStyle.MarginTop = middleState.MarginTop ? middleState.MarginTop : this.middleStyle.MarginTop
            this.middleStyle.MarginBottom = middleState.MarginBottom ? middleState.MarginBottom : this.middleStyle.MarginBottom
            this.middleStyle.bgColor = middleState.bgColor ? middleState.bgColor : this.middleStyle.bgColor
            this.middleStyle.heightOffset = middleState.heightOffset || middleState.heightOffset === 0 ? middleState.heightOffset : this.middleStyle.heightOffset
            this.middleStyle.miniPlayerHeight = middleState.miniPlayerHeight || middleState.miniPlayerHeight === 0 ? middleState.miniPlayerHeight : this.middleStyle.miniPlayerHeight

            this.middleStyle.height = window.innerHeight - 110 - this.middleStyle.miniPlayerHeight + this.middleStyle.heightOffset + 'px'

            this.middleStyle.overScroll = middleState.overScroll ? middleState.overScroll : this.middleStyle.overScroll
        },


        // Session & Device Manager
        // setInactiveSessionData(state, inactiveSessionData) {
        //     this.inactiveData = inactiveSessionData
        ...
        //         }
        //     } else {
        //         this.theNewestTracks = newestTracks
        //     }
        // },
        //
        // // HomePage - Trending Tracks
        // setTrendingLanguage(state, lang) {
        //     this.trendingLanguage = lang
        // },
        ...
        //         }
        //     } else {
        //         this.theTrendingTracks = trendingTracks
        //     }
        // },
        //
        // // HomePage - Popular Artists
        // setPopularArtistsLanguage(state, language) {
        //     this.popularArtistsLanguage = language
        // },
        ...
        //     this._collectionData = collectionHomeData
        ...
        setPlayerData(state, playerData) {
            if (playerData) {
                this.currentTrack = playerData.item
                this.dispatch('setMediaSessionData')
                this.playerData = playerData.item
                this.playerIndex = playerData.index
                this.playerQuery = playerData.query
                this.playerMeta = playerData.meta
                this.playerQueryParams = playerData.queryParams
                this.playerFrom = playerData.from
            } else {
                this.playerData = null
                this.playerIndex = null
                this.playerQuery = null
                this.playerMeta = null
                this.playerQueryParams = null
        ...
        setTrackDownloadFlags() {
            for (let [index] in this.playerTracks.tracks) {
                this.playerTracks.tracks[index].isDownloading = false
                this.playerTracks.tracks[index].isDownloaded = false
            }
        },
        ...
        //     this.playlistCategories[categoryObject.title] = categoryObject.data
        // },
        // setPlaylistSubCategory(state, category) {
        //     this.playlistSubCategories[category.title] = category.data
        // },
        // setSectionCollections(state, sectionCollectionsData) {
        //     this.playlistSubCategories[router.currentRoute.query.title].collections.push(...sectionCollectionsData.result.collections)
        //     this.playlistSubCategories[router.currentRoute.query.title].end = sectionCollectionsData.result.end
        // },
        // setCategoryCollections(state, categoryCollectionsData) {
        //     this.playlistSubCategories[router.currentRoute.query.title].collections.data.push(...categoryCollectionsData.result.collections)
        //     this.playlistSubCategories[router.currentRoute.query.title].collections.end = categoryCollectionsData.result.end
        // },
        // setPlaylistCategoryById(state,category) {
        //   this.playlistCategoryByIdData = category.collection_category
        // },

        // Playlist Mutations - thePlaylist
        // setPlaylistData(state, _playlistData) {
        //     if (_playlistData)
        //         this.playlistData = _playlistData.collection
        //     else
        ...
        //     this.playlistSortTitle = sort.sortTitle
        //     this.playlistSortId = sort.sort
        // },
        ...
        setArtistSortTitle(state, sort) {
            this.artistSortTitle = sort.sortTitle
            this.artistSortId = sort.sort
        },
        // setArtistGroup(state, artist_group) {
        //     this.artistGroup = artist_group
        // },
        // setArtistGroupArtists(state, artist_group_artists) {
        //     this.artistGroupArtists = artist_group_artists
        // },
        // User Mutations
        setUser(state, user) {
            this.user = user
        ...
        setUserObject(state, userObject) {
            this.userObject.device_token = userObject.device_token
            this.userObject.token = userObject.token
            this.userObject.user = userObject.user
            this.userObject.verified = userObject.verified
        },
        ...
        //     this.myMelodifyData = myMelodifyData
        ...
        //     this.emptyListLoadMessage = message
        // },
        //
        //
        // setUserCollectionSortTitle(state, sort) {
        //     this.userCollectionSortTitle = sort.sortTitle
        //     this.userCollectionSortId = sort.sort
        // },
        // setUserCollections(state, _userCollections) {
        //     this.userCollections = _userCollections
        // },
        ...
        //     }
        //     else {
        //         this.userCollectionTracks = null
        //     }
        // },
        // // User - AboutUs Page
        ...
        //     this.searchState.searchType = _searchState[0].searchType
        //     this.searchState.query = _searchState[0].query
        // },
        ...
        //                     if (this[searchResults.storeObject][searchResults.searched[1]].length !== 0) {
        //                         this[searchResults.storeObject].end = searchResults.searched[0].result.end
        //                         this[searchResults.storeObject].remaining = searchResults.searched[0].result.remaining
        //                         // for (let item in _tracks.tracks) {
        //                         //     this[payload.module][payload.storeObject].tracks.push(_tracks.tracks[item])
        //                         // }
        //                         this[searchResults.storeObject][searchResults.searched[1]].push(...searchResults.searched[0].result[searchResults.searched[1]])
        //
        //                     } else {
        //                         this[searchResults.storeObject] = searchResults.searched[0].result
        //                         this[searchResults.storeObject][searchResults.searched[1]].push(...searchResults.searched[0].result[searchResults.searched[1]])
        //
        //                     }
        //
        ...
        //             } else if (action === 'default') {
        //                 this[searchResults.storeObject] = searchResults.searched[0].result
        //                 this[searchResults.storeObject][searchResults.searched[1]] = searchResults.searched[0].result[searchResults.searched[1]]
        //             }
        //         } else {
        //             // this[payload.module][payload.storeObject] = payload.tracksData
        //         }
        //     }
        //     else {
        ...
        //         })
        //     else
        //         this.ticketingData = _ticketingData
        //
        // },
        ...
        // setFAQ(state, _FAQs) {
        //     if (_FAQs)
        //         this.FAQs = _FAQs.faq_categories
        //     else
        //         this.FAQs = _FAQs
        // }
        setLyricsLoading(context, payload) {
          this.isLoadingLyrics = payload
        },
        setLastAdvData(context,payload)
        {
          this.lastAdvData = payload
        },
        setListenCount(context,payload) {
            if(payload.command === 'reset') {
                ...
                this.listenCount += payload.size
            }
        },
        setPlaylistFollowed() {
            this.playlistData.is_followed = true
        },
        setPlaylistUnFollowed() {
            this.playlistData.is_followed = false
        },
        setArtistFollowed() {
            this.artistData.is_followed = true
        },
        setArtistUnFollowed() {
            this.artistData.is_followed = false
        },
        setMediaSessionData() {
        ...
        calcAdvertiseIndex(context, payload) {
            if (payload != 0) {
                ...
                this.advertiseIndex = payload
            }
        },
        setClosePlayer(context, payload) {
            this.should_close_player = payload
        },
        setLastListenedTrack(context, payload) {
            if(payload) {
                ...
                let isInPlayedList = this.lastListenedTracksArray.filter(item => item.track_id === payload.track_id)
                if (isInPlayedList.length < 1) {
                    this.lastListenedTracksArray.push(trackObject)
                } else {
                    this.lastListenedTracksArray = this.lastListenedTracksArray.filter(item => item.track_id !== payload.track_id)
                    this.lastListenedTracksArray.push(trackObject)
                }
                if (this.lastListenedTracksArray.length > 0) {
                    this.lastListenedTracksArray.sort((a, b) => {
                        if (a.timestamp < b.timestamp) return -1;
                        if (a.timestamp > b.timestamp) return 1;
                        return 0;
                    })
                    if (this.lastListenedTracksArray.length > this.maxListenableTracksCount) {
                        this.lastListenedTracksArray.splice(0, 1);
                        const toRemove = document.getElementById(this.lastListenedTracksArray[0].track_id)
                        if (toRemove) toRemove.remove()
                    }
                }
            }
            else {
                this.lastListenedTracksArray = []
            }
                ...
                switch (payload.target_type) {
                    ...
                    case 'track': {
                        Promise.all([getArtistTracks(payload.artist_id)]).then(() => {
                            let chosenTrack = this.playerTracks.tracks.filter(item => item.id.toString() === payload.track_id.toString())
                            console.log(chosenTrack)
                            let chosenTrackIndex = this.playerTracks.tracks.indexOf(chosenTrack[0])
                            console.log(chosenTrackIndex)
                            let el = document.getElementById('navWrap');
                            el.__vue__.$root.$refs.thePlayer.playItem(this.playerTracks.tracks, chosenTrack[0], chosenTrackIndex, 'getArtistTracks', {
                    ...
                    case 'handle_invalid_token' : {
                        ...
                        localStorage.clear()
                        // localStorage.setItem('invalid_token_data',this.invalidTokenData)
        ...
        setPlanBackState (context,payload) {
            this.planBackState = payload
        },
        empty_premium_dialog(context, payload) {
            this.appDialogs.premium_dialog = payload
        },
        empty_custom_dialog(context, payload) {
            this.appDialogs.custom_dialog = payload
        },
        setMusicListEditMode(context, payload) {
            if(payload)
            {
                this.playerCheckedItems = []
            }
            this.musicListEditMode = payload
        },
        setMusicListCheckedItems(context, payload) {
            if (!payload) {
                this.musicListCheckedItems = []
            } else {
                if(!Array.isArray(payload))
                {
                    this.musicListCheckedItems.push(payload)
                }
                else
                    {
                    this.musicListCheckedItems = payload
                }
            }
        },
        setPointedTrackID(context,payload) {
          this.currentPointedTrackID = payload
        },
        setDownloadedTracks(context,payload) {
          this.downloadedTracks = payload
        },
        setListenedTracks(context,payload) {
            this.listenedTracks = payload
        },
        setPlayerSelectionMode(context, payload) {
            ...
            this.playerSelectionMode = payload
        },
        setPlayerCheckedItems(context, payload)
        {
            if (!payload) {
                ...
                if(!Array.isArray(payload))
                {
                    this.playerCheckedItems.push(payload)
                }
                else
                {
                    this.playerCheckedItems = payload
                }
            }
        },
        async get_Data(context, pay_load) {
            ...
            // if(this.isOnline) {
                ...
                let userObject = this.devUserObject["09353264254"]
                let params = null
                if (payload.params) {
                ...
                await axios({
                    ...
                    url: (payload.BaseUrl ? payload.BaseUrl : this.activeUrl) + 'api/' + (payload.api_version ? payload.api_version : this.api_version) + payload.api_command,
                    ...
                    console.log((payload.BaseUrl ? payload.BaseUrl : this.activeUrl) + 'api/' + (payload.api_version ? payload.api_version : this.api_version) + payload.api_command)
                                    ...
                                    if (error.response.status && error.response.status === 406) {
                                        ...
                                        if (this.retryCounter < 3) {
                                            setTimeout(() => {
                                                ...
                                                console.log(this.retryCounter + ' atempt of retry')
                                                this.retryCounter++
